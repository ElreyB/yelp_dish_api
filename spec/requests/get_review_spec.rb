...
describe "get all reviews from single meal" do
  ...
  before { get "/api/v1/meals/#{meals.id}/reviews", headers: {Authorization: ENV['YELP_API_KEY']}}
...
describe "get single review from single meal" do
  ...
  before { get "/api/v1/meals/#{meals.id}/reviews/#{reviews.id}", headers: {Authorization: ENV['YELP_API_KEY']}}
